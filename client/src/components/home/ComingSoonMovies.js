...
const ComingSoonMovies = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        setLoading(true);
        const data = await movieService.getMoviesByStatus('Coming Soon');
  ...
  if (movies.length === 0) {
    return (
      <div className="bg-secondary text-white p-4 rounded-lg">
        <p>No upcoming movies at the moment. Check back soon!</p>
      </div>
    );
  }
      ...
      {movies.map((movie) => (
        <div key={movie._id} className="bg-secondary rounded-lg overflow-hidden shadow-lg transition-transform duration-300 hover:scale-105">
          ...
          <div className="p-4">
            ...
            <Link to={`/movies/${movie._id}`} className="btn btn-secondary w-full">
              More Info
            </Link>
          </div>
        </div>
