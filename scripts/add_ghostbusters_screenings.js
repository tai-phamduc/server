...
// Function to create multiple screenings for Ghostbusters: Frozen Empire
async function createGhostbustersScreenings() {
  try {
    ...
    if (!movie) {
      console.log(`Movie "${MOVIE_TITLE}" not found.`);
      mongoose.disconnect();
      return;
    }
    
    console.log(`Creating screenings for movie: ${movie.title} (ID: ${movie._id})`);
    ...
    const screeningConfigs = [
      ...
      // Friday evening, 4DX
      {
        ...
        startTime: (() => {
          ...
          const daysUntilFriday = (5 - time.getDay() + 7) % 7;
          time.setDate(time.getDate() + daysUntilFriday);
      ...
      // Sunday family matinee, 2D
      {
        ...
        format: '2D',
        basePrice: 8.5, // Special family matinee price
        cinema: cinemas[1] || cinemas[0],
        room: roomIds[1] || roomIds[0]
      }
    ];
    ...
    for (const config of screeningConfigs) {
      // Calculate end time (movie duration is 115 minutes)
      const endTime = new Date(config.startTime);
      endTime.setMinutes(endTime.getMinutes() + 115);
